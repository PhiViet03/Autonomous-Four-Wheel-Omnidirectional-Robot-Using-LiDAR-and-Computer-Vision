...
def reset_encoders_turn():
    global enc_turn_start
    enc_turn_start = enc_count.copy()

def rotate_ticks():
    s = 0
    for i in range(4):
        s += abs(enc_count[i] - enc_turn_start[i])
    return int(s / 4)

# ================= TURN =================
def do_turn(direction):
    hard_stop(1.0)   # <<< DỪNG TRƯỚC KHI QUAY
    ...
    while rotate_ticks() < TARGET_TICKS:
        print("ticks =", rotate_ticks())
        drive(0, 0, TURN_SIGN * W_ROT)
        time.sleep(dt)

    hard_stop(0.5)   # <<< DỪNG SAU KHI QUAY
...
def keyboard_listener():
    global lidar_trigger, exit_flag, mode, turn_dir
    while True:
        k = sys.stdin.read(1)
